...
public struct LuminareList<ContentA, ContentB, V, ID>: View
    where ContentA: View,
    ContentB: View, V: Hashable, ID: Hashable
{
...
struct LuminareListItem<Content, V>: View where Content: View, V: Hashable {
    ...
    init(
        ...
        _lastItem = lastItem
        _canRefreshSelection = canRefreshSelection
    }
        ...
        Color.clear
            ...
            .onChange(of: selection) { _ in
                ...
                DispatchQueue.main.async {
                    withAnimation(LuminareConstants.animation) {
                        tintOpacity = selection
                            .contains(item) ? maxTintOpacity : .zero
                        lineWidth = selection
                            .contains(item) ? maxLineWidth : .zero
                    }
                }
            }
    }
        ...
        Group {
            ...
            if isHovering {
                Rectangle()
                    ...
                    .opacity((maxTintOpacity - tintOpacity) *
                        (1 / maxTintOpacity))
            }
        }
    }
    ...
    func lastItemPart(isBottomOfList: Bool) -> some View {
        VStack(spacing: 0) {
            ...
            ZStack {
                UnevenRoundedRectangle(
                    topLeadingRadius: 0,
                    bottomLeadingRadius: isBottomOfList ?
                        (12 + lineWidth / 2.0) : cornerRadius,
                    bottomTrailingRadius: isBottomOfList ?
                        (12 + lineWidth / 2.0) : cornerRadius,
                    topTrailingRadius: 0
                )
                .strokeBorder(tintColor(), lineWidth: lineWidth)
    ...
    func singleSelectionPart(isBottomOfList: Bool) -> some View {
        UnevenRoundedRectangle(
            topLeadingRadius: cornerRadius,
            bottomLeadingRadius: isBottomOfList ? (12 + lineWidth / 2.0) :
                cornerRadius,
            bottomTrailingRadius: isBottomOfList ? (12 + lineWidth / 2.0) :
                cornerRadius,
            topTrailingRadius: cornerRadius
        )
        .strokeBorder(tintColor(), lineWidth: lineWidth)
